...
import { safeParseJson } from '../utils/safeApi';
  ...
  const verifyPassword = async (pass: string, isAutoCheck = false) => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        const storedPass = localStorage.getItem('admin_password') || 'Akk@071772022';
        if (pass === storedPass || pass === 'Akk@071772022') {
          setIsLoggedIn(true);
      ...
      if (pass === storedPass || pass === 'Akk@071772022') {
        ...
        if (!isAutoCheck) setLoginError('စကားဝှက် မှားယွင်းနေပါသည်');
        setIsLoggedIn(false);
      }
    } finally {
